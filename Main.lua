...
__haxe_crypto_Sha1 = _hx_e()
__haxe_io_Bytes = _hx_e()
__lua_Boot = _hx_e()
__processing_structures_Error = _hx_e()
__processing_structures_HyperLogLog = _hx_e()
__processing_structures_HyperLogLogPlusPlus = _hx_e()
__processing_structures_HyperLogLogPlusPlusData = _hx_e()
...
Main.calc = function(keys,args) 
  ...
  local bytes = __haxe_io_Bytes.alloc(1024);
  local hll = __processing_structures_HyperLogLogPlusPlus.new(14, nil);
  local _g3 = 0;
  while (_g3 < 100) do 
    _g3 = _g3 + 1;
    local i2 = _g3 - 1;
    hll:add(Std.string("") .. Std.string(i2));
  end;
  do return hll:count() end;
end
...
__haxe_Int64Helper.fromFloat = function(f) 
  ...
  while (rest >= 1) do 
    ...
    rest = rest / 2;
    if (curr >= 1) then 
      local this_high = __haxe__Int32_Int32_Impl_.shr(1, 31);
      local this_low = 1;
      local b = i;
      b = _hx_bit.band(b,63);
      local b1;
      if (b == 0) then 
        local this2 = __haxe__Int64____Int64.new(this_high, this_low);
        b1 = this2;
      else
        if (b < 32) then 
          local this3 = __haxe__Int64____Int64.new(__haxe__Int32_Int32_Impl_["or"](__haxe__Int32_Int32_Impl_.clamp(_hx_bit.lshift(this_high,b)), _hx_bit.rshift(this_low,32 - b)), __haxe__Int32_Int32_Impl_.clamp(_hx_bit.lshift(this_low,b)));
          b1 = this3;
        else
          local this4 = __haxe__Int64____Int64.new(__haxe__Int32_Int32_Impl_.clamp(_hx_bit.lshift(this_low,b - 32)), 0);
          b1 = this4;
        end;
      end;
      ...
      local this5 = __haxe__Int64____Int64.new(high, low);
      result = this5;
    end;
    i = i + 1;
  end;
  if (neg) then 
    local a = result.high;
    local high1 = _hx_bit_clamp(_hx_bit.bnot(a));
    local low1 = -result.low;
    if (low1 == 0) then 
      high1 = high1 + 1;
      local ret1 = high1 - 1;
      high1 = __haxe__Int32_Int32_Impl_.clamp(high1);
    end;
    local this6 = __haxe__Int64____Int64.new(high1, low1);
    result = this6;
  end;
  do return result end;
end

__haxe_crypto_Sha1.new = function() 
  local self = _hx_new(__haxe_crypto_Sha1.prototype)
  __haxe_crypto_Sha1.super(self)
  return self
end
__haxe_crypto_Sha1.super = function(self) 
end
__haxe_crypto_Sha1.make = function(b) 
  local h = __haxe_crypto_Sha1.new():doEncode(__haxe_crypto_Sha1.bytes2blks(b));
  local out = __haxe_io_Bytes.alloc(20);
  local p = 0;
  local _g = 0;
  while (_g < 5) do 
    _g = _g + 1;
    local i = _g - 1;
    p = p + 1;
    out.b[p - 1] = _hx_bit.band(_hx_bit.rshift(h[i],24),255);
    p = p + 1;
    out.b[p - 1] = _hx_bit.band(_hx_bit.band(_hx_bit.arshift(h[i],16),255),255);
    p = p + 1;
    out.b[p - 1] = _hx_bit.band(_hx_bit.band(_hx_bit.arshift(h[i],8),255),255);
    p = p + 1;
    out.b[p - 1] = _hx_bit.band(_hx_bit.band(h[i],255),255);
  end;
  do return out end;
end
__haxe_crypto_Sha1.bytes2blks = function(b) 
  ...
  while (_g11 < _g2) do 
    ...
    local blks1 = blks;
    local p1 = p;
    blks1[p1] = _hx_bit.bor(blks1[p1],_hx_bit.lshift(b.b[i1],24 - (_hx_bit.lshift((_hx_bit.band(i1,3)),3))));
  end;
  local i2 = b.length;
...
__haxe_crypto_Sha1.prototype = _hx_a();
__haxe_crypto_Sha1.prototype.doEncode = function(self,x) 
  local w = Array.new();
  local a = 1732584193;
  local b = -271733879;
  local c = -1732584194;
...
__haxe_crypto_Sha1.prototype.ft = function(self,t,b,c,d) 
  if (t < 20) then 
...
__haxe_crypto_Sha1.prototype.kt = function(self,t) 
  if (t < 20) then 
...
__processing_structures_Error.new = function(msg) 
  local self = _hx_new(__processing_structures_Error.prototype)
  __processing_structures_Error.super(self,msg)
  return self
end
__processing_structures_Error.super = function(self,msg) 
  self.msg = msg;
end
__processing_structures_Error.prototype = _hx_a();
__processing_structures_Error.prototype.toString = function(self) 
  do return Std.string("processing.structures.Error: ") .. Std.string(self.msg) end
end

__processing_structures_HyperLogLog.new = function(precision,register) 
  local self = _hx_new(__processing_structures_HyperLogLog.prototype)
  __processing_structures_HyperLogLog.super(self,precision,register)
  return self
end
__processing_structures_HyperLogLog.super = function(self,precision,register) 
  if (precision == nil) then 
    precision = 8;
  end;
  if (register == nil) then 
    if ((precision < 4) or (16 < precision)) then 
      _G.error(__processing_structures_Error.new("Invalid prevision value. It should be from range [4-16]."),0);
    end;
    self.precision = precision;
    self.registerSize = _hx_bit.lshift(1,precision);
    self.register = __haxe_io_Bytes.alloc(self.registerSize);
    self.register:fill(0, self.registerSize, 0);
  else
    self.register = register;
    self.registerSize = register.length;
    self.precision = self:bitLength(self.registerSize) - 1;
    if ((_hx_bit.lshift(1,self.precision)) ~= self.registerSize) then 
      _G.error(__processing_structures_Error.new("Invalid initial register size. Only power of 2 are available."),0);
    end;
  end;
  self.alpha = self:getAlpha(self.precision);
  self.maxRank = self:getHashRangeBits() - self.precision;
end
__processing_structures_HyperLogLog.prototype = _hx_a();
__processing_structures_HyperLogLog.prototype.getHashRangeBits = function(self) 
  do return 32 end
end
__processing_structures_HyperLogLog.prototype.hashString = function(self,s) 
  local hvBytes = __haxe_crypto_Sha1.make(__haxe_io_Bytes.ofString(s));
  local v = _hx_bit.bor(_hx_bit.bor(_hx_bit.bor(hvBytes.b[0],_hx_bit.lshift(hvBytes.b[1],8)),_hx_bit.lshift(hvBytes.b[2],16)),_hx_bit.lshift(hvBytes.b[3],24));
  local this1 = __haxe__Int64____Int64.new(0, _hx_bit_clamp((function() 
    local _hx_1
...
__processing_structures_HyperLogLog.prototype.add = function(self,s) 
  local hv = self:hashString(s);
  local x = self.registerSize - 1;
  local this_high = __haxe__Int32_Int32_Impl_.shr(x, 31);
  local this_low = x;
  local this_high1 = _hx_bit.band(hv.high,this_high);
  local this_low1 = _hx_bit.band(hv.low,this_low);
  if (this_high1 ~= __haxe__Int32_Int32_Impl_.shr(this_low1, 31)) then 
    _G.error("Overflow",0);
  end;
  local regIdx = this_low1;
  local rank = self.maxRank;
  local b = self.precision;
  b = _hx_bit.band(b,63);
  local rank1;
  if (b == 0) then 
    local this1 = __haxe__Int64____Int64.new(hv.high, hv.low);
    rank1 = this1;
  else
    if (b < 32) then 
      local this2 = __haxe__Int64____Int64.new(_hx_bit.rshift(hv.high,b), __haxe__Int32_Int32_Impl_["or"](__haxe__Int32_Int32_Impl_.clamp(_hx_bit.lshift(hv.high,32 - b)), _hx_bit.rshift(hv.low,b)));
      rank1 = this2;
    else
      local this3 = __haxe__Int64____Int64.new(0, _hx_bit.rshift(hv.high,b - 32));
      rank1 = this3;
    end;
  end;
  local rank2 = (rank - self:bitLength64(rank1)) + 1;
  if (rank2 <= 0) then 
    _G.error(__processing_structures_Error.new("Error occured durring adding item into HyperLogLog counter. Hash value overflow. Probably there is incorrect maxRank value set."),0);
  end;
  if (rank2 > self.register.b[regIdx]) then 
    self.register.b[regIdx] = _hx_bit.band(rank2,255);
  end;
end
__processing_structures_HyperLogLog.prototype.countEsitmatorValue = function(self) 
  local divisor = 0.0;
  local _g1 = 0;
  local _g = self.register.length;
  while (_g1 < _g) do 
    _g1 = _g1 + 1;
    local i = _g1 - 1;
    divisor = divisor + _G.math.pow(2.0, -self.register.b[i]);
  end;
  do return ((self.alpha * self.registerSize) * self.registerSize) / divisor end
end
__processing_structures_HyperLogLog.prototype.countWithLinearCount = function(self,zeroRegsNum) 
  if (zeroRegsNum == nil) then 
    zeroRegsNum = -1;
  end;
  if (zeroRegsNum < 0) then 
    zeroRegsNum = 0;
    local _g1 = 0;
    local _g = self.register.length;
    while (_g1 < _g) do 
      _g1 = _g1 + 1;
      local i = _g1 - 1;
      if (self.register.b[i] == 0) then 
        zeroRegsNum = zeroRegsNum + 1;
      end;
    end;
  end;
  do return __haxe_Int64Helper.fromFloat(_G.math.floor((self.registerSize * _G.math.log(self.registerSize / (1.0 * zeroRegsNum))) + 0.5)) end
end
__processing_structures_HyperLogLog.prototype.bitLength = function(self,v) 
  local result = 0;
  while (v > 0) do 
    result = result + 1;
    v = _hx_bit.rshift(v,1);
  end;
  do return result end
end
__processing_structures_HyperLogLog.prototype.bitLength64 = function(self,v) 
  local result = 0;
  local zero_high = 0;
  local zero_low = 0;
  while (true) do 
    local v1 = __haxe__Int32_Int32_Impl_.clamp(v.high - zero_high);
    v1 = (function() 
      local _hx_1
      if (v1 ~= 0) then 
      _hx_1 = v1; else 
      _hx_1 = __haxe__Int32_Int32_Impl_.ucompare(v.low, zero_low); end
      return _hx_1
    end )();
    if (not ((function() 
      local _hx_2
    ...
    b = _hx_bit.band(b,63);
    if (b == 0) then 
      local this1 = __haxe__Int64____Int64.new(v.high, v.low);
      v = this1;
    else
      if (b < 32) then 
        local this2 = __haxe__Int64____Int64.new(_hx_bit.rshift(v.high,b), __haxe__Int32_Int32_Impl_["or"](__haxe__Int32_Int32_Impl_.clamp(_hx_bit.lshift(v.high,32 - b)), _hx_bit.rshift(v.low,b)));
        v = this2;
      else
        local this3 = __haxe__Int64____Int64.new(0, _hx_bit.rshift(v.high,b - 32));
        v = this3;
      end;
    end;
  end;
...
__processing_structures_HyperLogLog.prototype.getAlpha = function(self,precision) 
  if ((precision < 4) or (16 < precision)) then 
    _G.error(__processing_structures_Error.new("Invalid prevision value. It should be from range [4-16]."),0);
  end;
  if (precision == 4) then 
  ...
  do return 0.7213 / (1.0 + (1.079 / (_hx_bit.lshift(1,precision)))) end
end

__processing_structures_HyperLogLogPlusPlus.new = function(precision,register) 
  local self = _hx_new(__processing_structures_HyperLogLogPlusPlus.prototype)
  __processing_structures_HyperLogLogPlusPlus.super(self,precision,register)
  return self
end
__processing_structures_HyperLogLogPlusPlus.super = function(self,precision,register) 
  __processing_structures_HyperLogLog.super(self,precision,register);
end
__processing_structures_HyperLogLogPlusPlus.prototype = _hx_a();
__processing_structures_HyperLogLogPlusPlus.prototype.getHashRangeBits = function(self) 
  do return 64 end
end
__processing_structures_HyperLogLogPlusPlus.prototype.estimateBias = function(self,e,p) 
  local biasVector = __processing_structures_HyperLogLogPlusPlusData.bias[p - 4];
  local estimateVector = __processing_structures_HyperLogLogPlusPlusData.rawEstimate[p - 4];
  local estimError = Array.new();
  local _g1 = 0;
  local _g = estimateVector.length;
  while (_g1 < _g) do 
    _g1 = _g1 + 1;
    local i = _g1 - 1;
    estimError:push(_hx_o({__fields__={pos=true,val=true,err=true},pos=i,val=estimateVector[i],err=_G.math.pow(e - estimateVector[i], 2)}));
  end;
  estimError:sort(function(a,b) 
    do return Std.int(a.err - b.err) end;
  end);
  local result = 0.0;
  local _g2 = 0;
  while (_g2 < 6) do 
    _g2 = _g2 + 1;
    local i1 = _g2 - 1;
    result = result + biasVector[estimError[i1].pos];
  end;
  do return result / 6 end
end
__processing_structures_HyperLogLogPlusPlus.prototype.hashString = function(self,s) 
  local hvBytes = __haxe_crypto_Sha1.make(__haxe_io_Bytes.ofString(s));
  local v = _hx_bit.bor(_hx_bit.bor(_hx_bit.bor(hvBytes.b[4],_hx_bit.lshift(hvBytes.b[5],8)),_hx_bit.lshift(hvBytes.b[6],16)),_hx_bit.lshift(hvBytes.b[7],24));
  ...
  local v1 = _hx_bit.bor(_hx_bit.bor(_hx_bit.bor(hvBytes.b[0],_hx_bit.lshift(hvBytes.b[1],8)),_hx_bit.lshift(hvBytes.b[2],16)),_hx_bit.lshift(hvBytes.b[3],24));
...
__processing_structures_HyperLogLogPlusPlus.prototype.count = function(self) 
  local nonZeroItemsNum = 0;
  local _g1 = 0;
  local _g = self.register.length;
  while (_g1 < _g) do 
    _g1 = _g1 + 1;
    local i = _g1 - 1;
    if (self.register.b[i] > 0) then 
      nonZeroItemsNum = nonZeroItemsNum + 1;
    end;
  end;
  local zeroItemsNum = self.registerSize - nonZeroItemsNum;
  if (zeroItemsNum > 0) then 
    local result = self:countWithLinearCount(zeroItemsNum);
    local b = __haxe_Int64Helper.fromFloat(__processing_structures_HyperLogLogPlusPlusData.thresholds[self.precision - 4]);
    local v = __haxe__Int32_Int32_Impl_.clamp(result.high - b.high);
    v = (function() 
      local _hx_1
      if (v ~= 0) then 
      _hx_1 = v; else 
      _hx_1 = __haxe__Int32_Int32_Impl_.ucompare(result.low, b.low); end
      return _hx_1
    end )();
    if ((function() 
      local _hx_2
  ...
  local e = self:countEsitmatorValue();
  if (e <= (5 * self.registerSize)) then 
    do return __haxe_Int64Helper.fromFloat(_G.math.floor((e - self:estimateBias(e, self.precision)) + 0.5)) end;
  end;
  do return __haxe_Int64Helper.fromFloat(_G.math.floor(e + 0.5)) end
end
__processing_structures_HyperLogLogPlusPlus.__super__ = __processing_structures_HyperLogLog
setmetatable(__processing_structures_HyperLogLogPlusPlus.prototype,{__index=__processing_structures_HyperLogLog.prototype})

__processing_structures_HyperLogLogPlusPlusData.new = {}
